...
struct GeminiSidePanel: View {
    ...
    // MARK: - Animation state (used by Messages extension)

    /// Tracks which message IDs have already animated in.
    @State var animatedMessageIDs: Set<String> = []
    /// Driving timer for thinking dot pulse.
    @State var dotTick: Bool = false
}
...
struct GeminiMessageRow: View {
    ...
    /// Slide direction for entrance — user messages enter from right, assistant from left.
    ...
    var body: some View {
        ...
        .transition(
            reduceMotion
                ? .opacity
                : .move(edge: slideEdge).combined(with: .opacity)
        )
    }

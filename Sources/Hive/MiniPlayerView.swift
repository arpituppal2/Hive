...
// and pause drive the tab's actual media elements via injected JS, and mute
// uses the same browser-level CEF mute as the tab pill (no fake rendering,
// no second renderer). Clicking the card returns to the tab.
//
...
struct MiniPlayerView: View {
    ...
    private func card(_ tab: BrowserState.Tab) -> some View {
    ...
    private func controlsRow(_ tab: BrowserState.Tab) -> some View {
        HStack(spacing: 10) {
            ...
            // Mute / unmute — the same browser-level CEF mute as the tab
            // pill, so the strip and the player always agree. The icon shows
            // the CURRENT state (slash = muted, click to unmute).
            Button(action: { state.toggleMiniPlayerMute() }) {
                Image(systemName: state.isMiniPlayerMuted ? "speaker.wave.2.fill" : "speaker.slash.fill")
                    .font(HiveDesign.Typography.sectionHeader)
                    .frame(width: 26, height: 26)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
            .accessibilityLabel(state.isMiniPlayerMuted ? "Unmute playback" : "Mute playback")
            .accessibilityHint("Toggles audio in the original tab")
            .help(state.isMiniPlayerMuted ? "Unmute" : "Mute")
    ...
    @ViewBuilder
    private func tabIcon(_ tab: BrowserState.Tab) -> some View {

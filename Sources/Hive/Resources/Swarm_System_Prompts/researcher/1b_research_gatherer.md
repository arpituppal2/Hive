...
> **Punch-up verdict (honest): SHIP OFF-THE-SHELF.** Search-execution discipline is rule-rich × deterministic — same shape as `retrieval_ranker` and `capture_scribe`, both verified NO_GAIN on distillation. This Cell ships `.instructOffTheShelf` until a held-out eval proves gain. Runtime wiring (ModelRole slot + ModelManifest entry + orchestrator dispatch) is a follow-on; this is the prompt-dev artifact.

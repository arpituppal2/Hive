    ...
    func body(content: Content) -> some View {
        content.background(
            GeometryReader { geo in
                Color.clear
                    .onAppear { frame = geo.frame(in: .named(BrowserState.peekCoordinateSpaceName)) }
                    .onChange(of: geo.frame(in: .named(BrowserState.peekCoordinateSpaceName))) { _, newFrame in
                        frame = newFrame
                    }
            }
        )
...
// BrowserState.previewPool (MRU, capped). The CefWebViews for every
...
struct TabPeekOverlay: View {
    ...
    var body: some View {
        ZStack {
            ...
            card
                ...
                .scaleEffect((state.activePeekTabID == nil && state.activePeekLinkURL == nil) ? 0.95 : 1.0)
                .opacity((state.activePeekTabID == nil && state.activePeekLinkURL == nil) ? 0 : 1)
                .animation(reduceMotion ? nil : HiveDesign.Animation.spring, value: state.activePeekTabID)
                .animation(reduceMotion ? nil : HiveDesign.Animation.spring, value: state.activePeekLinkURL)
        }
    }
    ...
    @ViewBuilder
    private func placeholder(for tab: BrowserState.Tab?) -> some View {
    ...
    private func header(_ tab: BrowserState.Tab) -> some View {
        HStack(spacing: 6) {
    ...
    private func footer(_ tab: BrowserState.Tab) -> some View {
        VStack(alignment: .leading, spacing: 0) {
    ...
    private func host(for tab: BrowserState.Tab) -> String {
        if let url = tab.model.url, let h = url.host { return h }
        return tab.savedURL?.host ?? "new tab"
    }

    private func isSecure(_ tab: BrowserState.Tab) -> Bool {
        tab.model.url?.scheme == "https" || tab.savedURL?.scheme == "https"
    }

    private func displayTitle(for tab: BrowserState.Tab) -> String {
    ...
    private func urlString(for tab: BrowserState.Tab) -> String {
        tab.model.url?.absoluteString ?? tab.savedURL?.absoluteString ?? ""
    }

    private func tabIndex(for tab: BrowserState.Tab) -> Int? {
    ...
    @ViewBuilder
    private func tabIcon(for tab: BrowserState.Tab) -> some View {

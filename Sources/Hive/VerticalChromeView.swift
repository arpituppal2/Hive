...
struct VerticalChromeView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .frame(height: HiveDesign.Zen.spaceRailHeight)
            ...
            // Essentials grid — Arc-style 64px tiles (spec §4). Uses
            // essentialTabs (isEssential only) so pinned tabs render exactly
            // once — as compact rows in the pinned zone below, never twice.
            if !state.essentialTabs.isEmpty {
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 64, maximum: 64))], spacing: HiveDesign.Space.xs) {
                    ForEach(state.essentialTabs) { tab in
            ...
            // Pinned zone — 32px bold rows (spec §4)
            if !state.pinnedTabs.isEmpty {
                ...
                .padding(.top, HiveDesign.Space.xs)
        ...
        .background(HiveDesign.Material.sidebarMaterial, in: Rectangle())
...
struct VerticalEssentialTile: View {
    ...
    var body: some View {
        Button(action: { state.selectTab(id: tab.id) }) {
            VStack(spacing: 4) {
                tabIcon.frame(width: 24, height: 24)
                Text(tab.model.title.isEmpty ? (tab.savedURL?.host ?? "App") : tab.model.title)
                    .font(HiveDesign.Typography.microLabelSecondary)
                    .foregroundStyle(isActive ? HiveDesign.Text.primary : HiveDesign.Text.tertiary)
                    .lineLimit(1)
            }
            .frame(width: HiveDesign.Zen.essentialsTile, height: HiveDesign.Zen.essentialsTile)
...
struct VerticalTabRow: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, HiveDesign.Zen.stripHPadding)
        .padding(.vertical, 0)
        .frame(height: HiveDesign.Zen.tabHeight)
        .scaleEffect(isHovered && !isActive ? HiveDesign.Zen.activeTabScale : 1.0)
        .animation(HiveDesign.Animation.snap, value: isHovered)
        .background(
    ...
    private var reorderableSiblings: [BrowserState.Tab] {
        state.tabs.filter {

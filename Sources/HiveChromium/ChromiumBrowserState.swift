...
@Observable
@MainActor
final class ChromiumBrowserState {
    ...
    func toggleTabGroup(id: UUID) {
        ...
        if !tabGroups[index].isCollapsed,
           !ChromiumHibernationAdapter.canCollapseGroup(
    ...
    init() {
        ...
        // ChromiumBrowserState outside the app lifecycle without CEF up.
    ...
    private func setupDefaults() {
        ...
        for index in tabGroups.indices {
            ...
            tabGroups[index].isCollapsed = ChromiumHibernationAdapter.restoredCollapseState(
    ...
    var isCurrentPageBookmarked: Bool {
        ...
        if urlString == ChromiumBrowserState.webChromeStartURL.absoluteString { return false }
    ...
    private struct SessionData: Codable, Sendable {
        ...
        init(layout: String, isCompactMode: Bool, showBookmarksBar: Bool, isMemorySaverEnabled: Bool = true, accentColorHex: String,
            ...
            self.userDefinedCommands = ChromiumBrowserState.normalizedUserDefinedCommands(userDefinedCommands)
        ...
        init(from decoder: Decoder) throws {
            ...
            userDefinedCommands = ChromiumBrowserState.normalizedUserDefinedCommands(
    ...
    private func runHibernationPass() {
        ...
        let candidates = tabs.compactMap { tab -> ChromiumHibernationAdapter.TabCandidate? in
            guard !tab.isHibernated, tab.id != splitSecondaryTabID,
                  ChromiumHibernationAdapter.shouldIncludeInCandidateSet(
                      tabID: tab.id,
                      isMRU: mruTabIDs.contains(tab.id),
                      collapsedGroupTabIDs: collapsedGroupTabIDs
                  ) else { return nil }
            let pageURL = ChromiumHibernationAdapter.effectiveWakeURL(
                currentURL: tab.model.url,
                savedURL: tab.savedURL
            )
            let hasPage = pageURL != nil
            return ChromiumHibernationAdapter.TabCandidate(
                id: tab.id,
        ...
        let approvedIDs = ChromiumHibernationAdapter.evaluate(
            tabs: candidates,
            activeTabID: activeTabID,
            activeWorkspaceID: currentWorkspaceID,
    ...
    private func hibernateTab(_ tab: Tab) -> Bool {
        ...
        tabObservationTasks["navigation-\(tab.id)"]?.cancel()
        guard let wakeURL = ChromiumHibernationAdapter.effectiveWakeURL(
            currentURL: tab.model.url,
            savedURL: tab.savedURL
        ) else {

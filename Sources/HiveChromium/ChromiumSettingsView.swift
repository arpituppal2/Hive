...
struct ChromiumSettingsView: View {
        ...
        VStack(alignment: .leading, spacing: 24) {
            settingsGroup("Tab Layout") {
                Picker("", selection: $state.layout) {
                    ForEach(ChromiumBrowserState.TabLayout.allCases) { layout in
                        HStack(spacing: 6) {
    ...
    private var searchTab: some View {
        VStack(alignment: .leading, spacing: 24) {
            settingsGroup("Search Engine") {
                VStack(spacing: 6) {
                    ForEach(ChromiumBrowserState.SearchEngine.allCases) { engine in
    ...
    private func searchEngineDescription(for engine: ChromiumBrowserState.SearchEngine) -> String {
        switch engine {
            ...
            settingsGroup("Web Research") {
                VStack(alignment: .leading, spacing: 10) {
                    Picker("Provider", selection: $state.researchProvider) {
                        ForEach(ChromiumBrowserState.ResearchProvider.allCases) { provider in
...
private struct SettingsSidebarRow: View {
    let tab: ChromiumSettingsView.SettingsTab
    let isSelected: Bool
    let action: () -> Void

...
public final class CefProfile {
    ...
    private func createContext() -> UnsafeMutablePointer<cef_request_context_t>? {
        ...
        guard let context = cef_request_context_create_context(&settings, nil) else { return nil }

        // Per-context scheme handler factories are NOT inherited from the
        // global context. Browsers created with this profile route custom
        // scheme requests through this context's own factory map, so every
        // scheme registered so far (the reserved `cefswift` bridge scheme
        // and any app schemes) must be re-registered here or those URLs
        // fail with ERR_UNKNOWN_URL_SCHEME.
        for entry in CefRuntime.shared.schemeHandlerEntriesForNewContexts() {
            registerSchemeHandler(scheme: entry.scheme, domain: entry.domain, handler: entry.handler, on: context)
        }
        return context
    }

    /// Registers `handler` for `scheme` on this profile's request context.
    ///
    /// Unlike ``CefRuntime/registerSchemeHandler(scheme:domain:handler:)``
    /// (which targets the global context), this targets the context this
    /// profile owns. Per-profile contexts do not inherit the global context's
    /// factories, so custom schemes served by the app must be registered on
    /// every profile that will display them. The reserved `cefswift` bridge
    /// scheme is re-registered automatically when the context is created.
    ///
    /// May be called any time after ``CefRuntime/initialize(configuration:)``;
    /// the context is created lazily on first use.
    public func registerSchemeHandler(
        scheme: String,
        domain: String? = nil,
        handler: some CefSchemeHandler
    ) {
        guard let context = makeRequestContext() else { return }
        defer { cefRelease(UnsafeMutableRawPointer(context)) }
        registerSchemeHandler(scheme: scheme, domain: domain, handler: handler, on: context)
    }

    /// Registers a scheme handler factory on an existing context.
    /// The context pointer is borrowed; the caller keeps ownership.
    private func registerSchemeHandler(
        scheme: String,
        domain: String? = nil,
        handler: some CefSchemeHandler,
        on context: UnsafeMutablePointer<cef_request_context_t>
    ) {
        let factory = SchemeHandlerFactory(handler: handler)
        // The context consumes our +1 factory ref.
        ...
        let registered = CefStringUtil.withCefString(scheme) { cefScheme in
            if let domain {
                return CefStringUtil.withCefString(domain) { cefDomain in
                    context.pointee.register_scheme_handler_factory?(context, cefScheme, cefDomain, factoryPointer)
                }
            }
            return context.pointee.register_scheme_handler_factory?(context, cefScheme, nil, factoryPointer)
        }
        if registered == 0 {
            FileHandle.standardError.write(
                Data("CefSwift: request-context scheme handler factory registration failed for scheme '\(scheme)'.\n".utf8))
        }
    }
}

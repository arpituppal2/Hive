...
public final class CefRuntime {
    ...
    /// Handlers registered on the global context, kept so every request
    /// context created afterwards can inherit them. Per-profile contexts do
    /// not inherit the global context's factories (CEF only routes scheme
    /// requests through the factory map of the context the browser was
    /// created with), so contexts must re-register each handler themselves.
    private var contextSchemeHandlerEntries: [(scheme: String, domain: String?, handler: any CefSchemeHandler)] = []

    /// Snapshot of the global scheme handlers for replay onto a newly
    /// created request context. MainActor-isolated like the rest of CefRuntime.
    func schemeHandlerEntriesForNewContexts() -> [(scheme: String, domain: String?, handler: any CefSchemeHandler)] {
        contextSchemeHandlerEntries
    }
    ...
    ///
    /// The registration applies to the global context immediately and to
    /// every per-profile request context created from then on.
    public func registerSchemeHandler(scheme: String, domain: String? = nil, handler: some CefSchemeHandler) {
        ...
        contextSchemeHandlerEntries.removeAll { $0.scheme == scheme && $0.domain == domain }
        contextSchemeHandlerEntries.append((scheme, domain, handler))

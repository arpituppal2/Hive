...
| I. AppKit graph rewrite | Requested NSScrollView + HiveGraphCanvasView architecture | **PARTIAL** | Code: `Sources/HiveMacApp/HiveGraphCanvasView.swift` (node/edge draw, hit-testing, first-pass label collision suppression, staged reveal timer), `Sources/HiveMacApp/HiveAppKitGraphSurface.swift` (NSScrollView wrapper + cursor-centered magnify path), integration toggle in `Sources/HiveMacApp/HiveMacRootView.swift` and settings toggle in `Sources/HiveUI/HiveOverlaySurfaces.swift` | Remaining parity is runtime interaction quality/tuning and default-path switchover validation on macOS | Validate and tune on macOS, then switch default path after acceptance checks |

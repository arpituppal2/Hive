...
1249 tests / 144 suites pass. Ships as a native macOS .dmg.
...
### Short Description
```
Hive is a Chromium-based browser with on-device AI (Apple MLX), a multi-model council that deliberates in parallel, agentic browsing via CDP, and deep research with inline citations. Zero telemetry. Zero tracking. 1249 tests pass.
```
...
We're at 1249 tests, 44 features, and the .dmg is ready to download. I'd love feedback on the Model Council flow — does seeing multiple models disagree help or confuse?
...
Zero telemetry. Zero tracking. 1249 tests pass. macOS only for now.
...
5. **Open source** — MIT license, 1249 tests, buildable from source

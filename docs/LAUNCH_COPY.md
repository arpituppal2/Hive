...
1,861 tests / 179 suites pass. Builds as a native macOS ad-hoc DMG for local development; Developer ID signing and notarization are still required for a distributable release.
...
The launch snapshot was at 1,861 tests, with a local ad-hoc .dmg ready for development validation—not yet distribution-ready. I'd love feedback on the Model Council flow — does seeing multiple models disagree help or confuse?
...
Zero telemetry. Zero tracking. The launch snapshot passed 1,861 tests. The current DMG is ad-hoc for local development; macOS only for now.
...
5. **Open source** — MIT license, 1,861 tests in the launch snapshot, buildable from source (current DMGs are ad-hoc development artifacts)

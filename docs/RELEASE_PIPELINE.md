...
**Status:** Local validation current — **1,782 tests / 174 suites green**, product build, ad-hoc bundle, preflight, and bootstrap smoke all pass. Distribution-only items below remain credential-gated and are not implied by the local artifact.
...
10 extensions), and`SettingsView` (770 → 83 + 7 extensions) followed with `scripts/split_swift_type.py` (commit `371fddcd`). These were pure refactors with zero behavior change; the decomposition checkpoint had 1,567 tests, and the current suite is 1,782 tests / 174 suites.

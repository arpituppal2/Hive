...
PRODUCT="HiveChromium"
APP_NAME="HiveChromium.app"
ALLOW_ADHOC=0
ENTITLEMENTS_PATH=""

usage() {
  cat <<'USAGE'
Assemble a signed HiveChromium app bundle for local/release validation.

Usage:
  scripts/build-hivechromium-app.sh [--output DIR] [--sign IDENTITY]

Options:
  --output DIR   Directory for HiveChromium.app (default: ./dist).
...
fail() {
  printf 'build-hivechromium-app: %s\n' "$1" >&2
  exit 1
}
...
if [[ -n "$ENTITLEMENTS_PATH" ]]; then
  ...
  "$ROOT_DIR/scripts/verify-hivechromium-entitlements.sh" "$ENTITLEMENTS_PATH"
fi
...
printf '%s\n' '==> Building HiveChromium SwiftPM resources'
...
RESOURCE_SOURCE="$BUILD_BIN_DIR/Hive_HiveChromium.bundle"
...
ICON_SOURCE="$ROOT_DIR/Sources/HiveChromium/Resources/AppIcon.icns"
...
for helper in \
  ...
  "HiveChromium Helper (Renderer).app"; do
...
sign_path "$APP_PATH/Contents/MacOS/HiveChromium" --sign "$SIGNING_IDENTITY"
sign_path "$APP_PATH" --sign "$SIGNING_IDENTITY"

printf '%s\n' '==> Running HiveChromium structural release preflight'
if [[ "$ALLOW_ADHOC" -eq 1 ]]; then
  "$ROOT_DIR/scripts/preflight-hivechromium-app.sh" --app "$APP_PATH" --allow-adhoc
else
  "$ROOT_DIR/scripts/preflight-hivechromium-app.sh" --app "$APP_PATH"
fi

if [[ "$ALLOW_ADHOC" -eq 1 ]]; then
  printf '%s\n' '==> Skipping release-only worker requirement verification (local ad-hoc mode)'
else
  printf '%s\n' '==> Verifying signed HiveChromium worker release contract'
  HIVE_WORKER_TEAM_ID="$TEAM_ID" \
...
if [[ "$ALLOW_ADHOC" -eq 1 ]]; then
  printf 'Built local ad-hoc HiveChromium app: %s\n' "$APP_PATH"
  printf '%s\n' 'This artifact is not signed for distribution, cannot satisfy the release worker requirement, and must not be notarized.'
else
  printf 'Built signed HiveChromium app: %s\n' "$APP_PATH"
  printf '%s\n' 'Notarization/stapling is intentionally not performed by this local assembly scaffold.'
  printf '%s\n' 'A notarization workflow still requires the final hardened-runtime/entitlements policy and a positive signed-fixture test.'
fi

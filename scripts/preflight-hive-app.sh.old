...
usage() {
  cat <<'USAGE'
Audit a HiveChromium.app bundle without modifying it.

Usage:
  scripts/preflight-hivechromium-app.sh [--app PATH] [--allow-adhoc]

Options:
  --app PATH       Bundle to inspect (default: ./dist/HiveChromium.app).
...
fail() {
  printf 'preflight-hivechromium-app: %s\n' "$1" >&2
  exit 1
}
...
MAIN_EXECUTABLE="$CONTENTS/MacOS/HiveChromium"

[[ -f "$CONTENTS/Info.plist" ]] || fail "missing Contents/Info.plist"
[[ -x "$MAIN_EXECUTABLE" ]] || fail "missing or non-executable HiveChromium binary"
...
[[ "$(plist_value "$CONTENTS/Info.plist" CFBundleExecutable)" == "HiveChromium" ]] || \
  fail "CFBundleExecutable is not HiveChromium"
...
EXPECTED_NESTED_BUNDLES=(
  "$CEF_FRAMEWORK"
  "$FRAMEWORKS/HiveChromium Helper.app"
  "$FRAMEWORKS/HiveChromium Helper (Alerts).app"
  "$FRAMEWORKS/HiveChromium Helper (GPU).app"
  "$FRAMEWORKS/HiveChromium Helper (Plugin).app"
  "$FRAMEWORKS/HiveChromium Helper (Renderer).app"
)
...
done < <(find "$RESOURCES" -type d -name '*_HiveChromium.bundle' -path '*/Contents/Resources/*_HiveChromium.bundle' -print0)
[[ "${#bundle_candidates[@]}" -eq 1 ]] || fail "expected exactly one HiveChromium SwiftPM resource bundle"

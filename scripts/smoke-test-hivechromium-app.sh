...
usage() {
  cat <<'USAGE'
Run the opt-in HiveChromium bootstrap smoke test.

Usage:
  scripts/smoke-test-hivechromium-app.sh [--app PATH]

Options:
  --app PATH       App bundle to launch (default: ./dist/HiveChromium.app).
...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hivechromium-smoke.XXXXXX")"
USER_DATA_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hivechromium-user-data.XXXXXX")"
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "ERROR: HiveChromium smoke test requires macOS (Darwin)." >&2
  exit 2
fi
...
if [[ ! -d "$APP_PATH" ]]; then
  ...
  echo "Build it first with: scripts/build-hivechromium-app.sh --allow-adhoc" >&2
  exit 2
fi

APP_PATH="$(cd "$APP_PATH" && pwd)"
APP_BINARY="$APP_PATH/Contents/MacOS/HiveChromium"
...
wait_for_readiness() {
  ...
  while [[ "$(date +%s)" -lt "$deadline" ]]; do
    ...
    if grep -q '^HIVE_READINESS_FAIL ' "$STDOUT_LOG" 2>/dev/null; then
      echo "FAIL: HiveChromium reported readiness failure." >&2
      exit 1
    fi
    ...
    if ! is_live_process "$APP_PID"; then
      echo "FAIL: HiveChromium exited before readiness." >&2
      exit 1
    fi
    sleep 0.2
  done
  ...
  if [[ "$readiness_seen" -ne 1 ]]; then
    echo "FAIL: HiveChromium readiness marker was not observed within ${TIMEOUT_SECONDS}s." >&2
    exit 1
  fi

  if ! is_live_process "$APP_PID"; then
    echo "FAIL: HiveChromium exited immediately after readiness." >&2
    exit 1
  fi
}
...
echo "PASS: HiveChromium emitted readiness within ${TIMEOUT_SECONDS}s."
